...
class MicrosoftEvent(abc.Set):
    ...
    def _load_odoo_ids_from_metadata(self, env):
        ...
        # Hence, we may have two Microsoft recurrences linked to the same dosyt id.
        # Therefore, we only consider dosyt records without microsoft id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Microsoft and dosyt.
        ...
        # (he is the orginizer in Microsoft). The "real" owner (in dosyt) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Microsoft: 1 email = 1 user; but in dosyt several users might have the same email
