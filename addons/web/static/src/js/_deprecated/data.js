 ...
 * consumed by dosytERP's RPC APIs.
 *
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of dosytERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the dosytERP model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // dosytERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // dosytERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, dosytERP domain
     ...
     * an SQL-like form usable by dosytERP's ORM.
     *
...
var Model = Class.extend({
    ...
    @param {String} name name of the dosytERP model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound dosytERP model.
     *
