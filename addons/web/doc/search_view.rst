...
dosytERP Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (dosytERP
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the dosytERP Web 7
...
The primary component of search views, as with all other dosytERP
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from dosytERP field types (and widgets) to search view
...
dosytERP this is done via :ref:`domains <openerpserver:domains>`. On
the other hand, the dosytERP Web 7 search view's state is modelled
...
:js:class:`~openerp.web.search.Facet` objects to dosytERP search data.
...
of the peculiarities pertaining to dosytERP's handling of fields in
...
dosytERP Web 6.1 to dosytERP Web 7.
