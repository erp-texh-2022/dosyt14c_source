...
A very basic dosytERP module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An dosytERP
...
.. note::
    ...
    restart the dosytERP server itself for it to be taken in account.
...
provides: an instance of the dosytERP Web client, with the contents of
...
will require restarting the dosytERP Server to see the changes, as
usual):
...
.. note::
    ...
    will apply only within the confines of the dosytERP Web client) and
...
dosytERP Web's template language is :doc:`qweb`. Although any
...
integration to dosytERP Web widgets.
...
To prevent this irremediable loss, we can use dosytERP's support for
...
basic dosytERP model in which our data will be stored:
...
4. And finally — after restarting dosytERP — navigating to the test
